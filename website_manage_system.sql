...
CREATE TABLE `department`  (
  ...
  `department_total_people` int(5) UNSIGNED NOT NULL COMMENT '部门总人数',
  `department_total_website` int(5) UNSIGNED NOT NULL COMMENT '部门总网站数',
...
CREATE TABLE `log`  (
  ...
  `log_operation` tinyint(1) UNSIGNED NOT NULL COMMENT '操作类型1：登录 2：提交申请 3：审核通过 4：审核未通过',
  `log_additional` int(10) UNSIGNED NULL DEFAULT NULL COMMENT '如果是申请相关的则附加申请表id',
  `log_creat_time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '日志创建时间',
...
CREATE TABLE `user`  (
  ...
  `user_gender` tinyint(1) UNSIGNED NOT NULL DEFAULT 1 COMMENT '性别 1：男 0:女',
  `user_department_id` int(5) UNSIGNED NOT NULL COMMENT '用户所属部门，与部门表相关联',
  `user_office_phone` varchar(11) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '办公电话',
  `user_tel` varchar(11) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '手机电话',
